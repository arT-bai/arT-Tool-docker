...
> 一个基于 Rust 的轻量表格工具镜像：支持“数据清洗”和“格式转换（Word 表格转 Excel）”两类能力。

## 目录

- [项目简介](#项目简介)
- [功能亮点](#功能亮点)
- [今日改动 2026-04-19](#今日改动-2026-04-19)
- [技术栈](#技术栈)
- [快速开始](#快速开始)
...
- 新增“格式转换”后端接口：`POST /convert`
  - 仅接收 `.docx`
  - 自动解析 Word 表格并按“每张表 -> 1 个 `.xlsx`”导出
  - 多个 Excel 自动打包为 `.zip`，通过 `download_link` 下载
- 新增清洗分步接口：
  - `POST /table-headers`：上传后提取表头并返回 `upload_token`
  - `POST /preview/session`：基于 `upload_token` 按配置生成预览
- 导航与页面结构更新：
  - 首页为“格式转换”（`/`）
  - “数据清洗”页面路径为 `/wash`
...
- **文件处理**: `csv`, `calamine`, `rust_xlsxwriter`, `quick-xml`
...
- `GET /`：格式转换页面
- `GET /wash`：数据清洗页面
- `POST /table-headers`：上传文件并提取表头（清洗流程第 1 步）
- `POST /preview/session`：基于 `upload_token` 生成预览（清洗流程第 2 步）
- `POST /clean`：执行清洗并导出（清洗流程第 3 步）
- `POST /convert`：执行 Word 表格转换并导出压缩包
- `GET /download/:job_id`：下载结果文件（`.xlsx` 或 `.zip`）
...
- Content-Type: `multipart/form-data`
- 字段说明：
  - `file`: Word 文档（仅支持 `.docx`）

成功响应示例：

```json
{
  "output_filename": "demo_01_20260419_225439.zip",
  "table_count": 3,
  "download_link": "/download/xxx"
}
```
